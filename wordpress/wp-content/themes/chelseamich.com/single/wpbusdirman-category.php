...
<div id="text" class="cat-view">
		...
		<?php wpbusdirman_catpage_title();?>		
			...
			<?php if(empty($premier_listings) && empty($enhanced_listings) && empty($standard_listings)) { ?>
				<p>Sorry, there aren't any listings in this category yet.  Own a business in that fits this category?  <a href="/directory/submit-listing">Add it!</a></p>
			<?php } ?>
			
			
			<!-- List premier listings -->
	<?php if(!empty($premier_listings)) { ?> <div class="directory-listing-bar premier"> </div> <?php } ?>
				...
				<div class="directory-listing premier">
					...
					<div class="listing-info">
						...
						<div class="address">
							...
							<?php if(get_post_meta($post->ID, 'Business Website Address', true)) {echo ' &nbsp; &nbsp; | &nbsp; &nbsp; ';echo '<a href="'.get_post_meta($post->ID, 'Business Website Address', true).'">'.get_post_meta($post->ID, 'Business Website Address', true).'</a>';}?>
						</div>
					</div>
					<br class="clearer" />
				</div>
			...
			<?php if(!empty($enhanced_listings)) { ?> <div class="directory-listing-bar enhanced"> </div> <?php } ?>
					...
					<div class="listing-info">
						...
						<p><?php $text = get_the_excerpt(); echo truncate($text, 250); ?>  <a class="more-link" href="<?php the_permalink() ?>">More</a></p>
							...
							<div class="address">
								...
								<?php echo 'Tel: '; echo get_post_meta($post->ID, 'Business Phone Number', true); ?>
								<br />
								<?php if(get_post_meta($post->ID, 'Business Website Address', true)) {echo '<a href="'.get_post_meta($post->ID, 'Business Website Address', true).'">'.get_post_meta($post->ID, 'Business Website Address', true).'</a>';}?>
							</div>
					</div>
					<br class="clearer" />
				</div>
			...
			<?php if(!empty($standard_listings)) { ?> <div class="directory-listing-bar standard"> </div> <?php } ?>
