...
Plugin URI: http://www.themestown.com/wp-business-directory-manager
...
if(isset($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_51']) && !empty($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_51'])){$wpbdmposttypetagslug=$wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_51'];}else {$wpbdmposttypetagslug=$wpbdmposttypetags;}
			  ...
			  $args = array(
			    ...
			    'hierarchical' => false,
			    'menu_position' => null,
			    'supports' => array('title','editor','author','categories','tags','thumbnail','excerpt','comments','custom-fields')
			  );
			  register_post_type($wpbdmposttype,$args);

	//Register directory category taxonomy
	 register_taxonomy( $wpbdmposttypecategory, $wpbdmposttype, array( 'hierarchical' => true, 'label' => 'Directory Categories', 'update_count_callback' => '_update_post_term_count','query_var' => true, 'rewrite' => array('slug'=>$wpbdmposttypecategoryslug,'with_front'=>false) ) );
	 register_taxonomy( $wpbdmposttypetags, $wpbdmposttype, array( 'hierarchical' => false, 'label' => 'Directory Tags', 'update_count_callback' => '_update_post_term_count', 'query_var' => true, 'rewrite' => array('slug'=>$wpbdmposttypetagslug,'with_front'=>false) ) );
...
{
	echo "<p><b>";
	_e("Installed Payment Gateway Modules","WPBDM");
	echo "</b><ul>";
		if($wpbusdirman_hasgooglecheckoutmodule == 1){
		echo "<li style=\"background:url($wpbusdirman_imagesurl/check.png) no-repeat left center; padding-left:30px;\">";
		_e("Google Checkout","WPBDM");
		echo "</li>";
		}
		if($wpbusdirman_haspaypalmodule == 1){
		echo "<li style=\"background:url($wpbusdirman_imagesurl/check.png) no-repeat left center; padding-left:30px;\">";
	...
	{?>
	<?php if($wpbusdirman_hastwocheckoutmodule != 1 || $wpbusdirman_haspaypalmodule != 1 ){?>
	<div style="width:100%;padding:10px;">
	<?php if(!($wpbusdirman_haspaypalmodule == 1)){?>
	<div style="float:left;width:30%;padding:10px;">
	<?php
	_e("You can buy the PayPal gateway module to add PayPal as a payment option for your users.","WPBDM");?>
	<span style="color:red;font-weight:bold;text-transform:uppercase;">
	<?php _e("$49.99","WPBDM");?>
	</span>
	<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
	<input type="hidden" name="cmd" value="_s-xclick">
	<input type="hidden" name="hosted_button_id" value="YU3X22KHQ53P8">
	<input type="image" src="https://www.paypal.com/en_US/i/btn/btn_buynowCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
	<img alt="" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
	</form>
	</div>
	<?php } ?>
	<?php if(!($wpbusdirman_hastwocheckoutmodule == 1)){?>
	<div style="float:left;width:30%;padding:10px;">
	<?php
	_e("You can buy the 2Checkout gateway module to add 2Checkout as a payment option for your users.","WPBDM");?>
	<span style="color:red;font-weight:bold;text-transform:uppercase;">
	<?php _e("$49.99","WPBDM");?>
	</span>
	<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
	<input type="hidden" name="cmd" value="_s-xclick">
	<input type="hidden" name="hosted_button_id" value="U6T9ZMBB3HWDL">
	<input type="image" src="https://www.paypal.com/en_US/i/btn/btn_buynowCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
	<img alt="" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
	</form>
	</div>
	...
	<div style="float:left;width:30%;padding:10px;">
	<span style="color:red;font-weight:bold;text-transform:uppercase;">
	<?php _e("Save $20","WPBDM");?>
	</span>
	<?php _e(" on your purchase of both the Paypal and the 2Checkout gateway modules","WPBDM");?>
	<span style="color:red;font-weight:bold;text-transform:uppercase;">
	<?php _e("$79.98","WPBDM");?>
	</span>
	<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
	<input type="hidden" name="cmd" value="_s-xclick">
	<input type="hidden" name="hosted_button_id" value="KG5MFBC2XAXKW">
	<input type="image" src="https://www.paypal.com/en_US/i/btn/btn_buynowCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
	<img alt="" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
	</form>
	</div>
	...
	{
		_e("It does not appear you have any of the payment gateway modules installed. You need to purchase a payment gateway module in order to charge a fee for listings. To purchase payment gateways use the buttons below or visit","WPBDM");
		echo "<p><a href=\"http://themestown.com/business-directory-plugin/premium-modules\">http://themestown.com/business-directory-plugin/premium-modules</a></p>";
...
function wpbusdirman_gpid(){
	global $wpdb;
	$wpbusdirman_pageid = $wpdb->get_var("SELECT ID FROM $wpdb->posts WHERE post_content LIKE '%[WPBUSDIRMANUI]%' AND post_status='publish'");
...
function wpbusdirman_display_postform_add()
{
	...
	if($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_3'] == "yes")
	{
		if(!is_user_logged_in())
		{
			...
			echo "</p>";
			echo "<p><form method=\"post\" action=\"$wpbusdirman_loginurl\"><input type=\"submit\" class=\"insubmitbutton\" value=\"";
			_e("Login Now","WPBDM");
			echo "\"></form></p>";
		}
		else
...
function wpbusdirman_buildform()
{
	...
	elseif(($wpbusdirmanaction == 'deletefield'))
	{
		if(isset($_REQUEST['id']) && !empty($_REQUEST['id']))
		{
			...
			delete_option('wpbusdirman_postform_field_label_'.$wpbusdirman_fieldid_todel);
			delete_option('wpbusdirman_postform_field_type_'.$wpbusdirman_fieldid_todel);
			delete_option('wpbusdirman_postform_field_options_'.$wpbusdirman_fieldid_todel);
			delete_option('wpbusdirman_postform_field_order_'.$wpbusdirman_fieldid_todel);
			delete_option('wpbusdirman_postform_field_association_'.$wpbusdirman_fieldid_todel);
			delete_option('wpbusdirman_postform_field_validation_'.$wpbusdirman_fieldid_todel);
			delete_option('wpbusdirman_postform_field_required_'.$wpbusdirman_fieldid_todel);
			delete_option('wpbusdirman_postform_field_showinexcerpt_'.$wpbusdirman_fieldid_todel);
			...
			if($wpbusdirman_field_vals)
			{
				foreach($wpbusdirman_field_vals as $wpbusdirman_field_val)
				{
					...
					if(isset($wpbusdirmanerrors) && !empty($wpbusdirmanerrors))
					{
						...
						{
							$wpbusdirman_field_value=$_REQUEST['cat'];
						}
						else
						{
					...
					elseif($wpbusdirman_field_type == 2)
					{
							...
							if($wpbusdirman_field_association == 'category')
							{
								...
								wp_dropdown_categories('taxonomy='.$wpbdmposttypecategory.'&show_option_none='.$wpbusdirman_selectcattext.'&orderby=name&selected='.$wpbusdirman_field_value.'&order=ASC&hide_empty=0&selected='.$wpbusdirman_field_value.'&hierarchical=1');
							}
							else
							{
					...
					elseif($wpbusdirman_field_type == 6)
					{
						echo "<p class=\"wpbdmp\"><label>$wpbusdirman_field_label_name</label></p>";
							...
							if($wpbusdirman_formselop)
							{
								...
								{
									$wpbusdirman_formseloption=trim($wpbusdirman_formseloption);
									echo "<span style=\"padding-right:10px;\"><input type=\"checkbox\" name=\"wpbusdirman_field_label".$wpbusdirman_field_label_association."[]\" value=\"$wpbusdirman_formseloption\"";
									if(in_array($wpbusdirman_formseloption,$wpbusdirmanxboxvalsarr)){echo $wpbusdirmancheckedword;}
									echo "/>$wpbusdirman_formseloption</span>";
								}
							}
					}
...
function wpbusdirman_validate_data()
{
		...
		if($wpbusdirman_field_item_array)
		{
			foreach($wpbusdirman_field_item_array as $wpbusdirman_field_x_field)
			{
					...
					if($wpbusdirman_field_association == 'category')
					{
						$wpbusdirman_field_inputname="cat";
					}
					else
					{
					...
					elseif(($wpbusdirman_field_validation == 'url') && ($wpbusdirman_field_required == 'yes'))
					{
						...
						else
						{
							if(!wpbusdirmancreatethumb($wpbusdirmanuploadedfilename,$wpbusdirmanuploaddir,$wpbusdirmanthumbnailwidth))
							{
								...
								if($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_21'] == "yes")
								{
									...
									add_post_meta($wpbusdirmanlistingpostid, "costoflisting", $wpbusdirmanlistingcost, true) or update_post_meta($wpbusdirmanlistingpostid, "costoflisting", $wpbusdirmanlistingcost);
...
function wpbusdirman_load_payment_page($wpbusdirmanlistingpostid,$wpbusdirmanfeeoption)
{
	...
	if(($wpbusdirman_hasgooglecheckoutmodule == 1) && ($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_40'] != "yes"))
	{
		echo "<h4 class=\"paymentheader\">";
		_e("Pay Listing Fee via Google Checkout","WPBDM");
		echo "</h4>";
		$wpbusdirman_googlecheckout_button=wpbusdirman_googlecheckout_button($wpbusdirmanlistingpostid,$wpbusdirmanfeeoption);
		echo "<div class=\"paymentbuttondiv\">";
		echo $wpbusdirman_googlecheckout_button;
		echo "</div>";
	}

	if(($wpbusdirman_haspaypalmodule == 1) && ($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_41'] != "yes"))
	{
		echo "<h4 class=\"paymentheader\">";
		_e("Pay Listing Fee via PayPal","WPBDM");
		echo "</h4>";
		$wpbusdirman_paypal_button=wpbusdirman_paypal_button($wpbusdirmanlistingpostid,$wpbusdirmanfeeoption,$wpbusdirman_imagesurl);
		echo "<div class=\"paymentbuttondiv\">";
		echo $wpbusdirman_paypal_button;
		echo "</div>";
	}

	if(($wpbusdirman_hastwocheckoutmodule == 1)&& ($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_43'] != "yes"))
	{
		echo "<h4 class=\"paymentheader\">";
		_e("Pay Upgrade Fee via 2Checkout","WPBDM");
		echo "</h4>";
		$wpbusdirman_twocheckout_button=wpbusdirman_twocheckout_button($wpbusdirmanlistingpostid,$wpbusdirmanfeeoption,$wpbusdirman_gpid);
		echo "<div class=\"paymentbuttondiv\">";
		...
		foreach($wpbusdirman_settings_fees_ops as $wpbusdirman_settings_fees_op)
		{
			...
			if($wpbusdirman_savedpostcatitems)
			{
				...
				if(in_array($post_category_item, $wpbusdirman_savedpostcatitems))
				{
					...
					$wpbusdirman_fee_to_pay_li.="<li><input type=\"radio\" name=\"whichfeeoption\" value=\"$wpbusdirman_settings_fees_op\" checked />$wpbusdirman_fee_op_name $wpbusdirman_get_currency_symbol$wpbusdirman_get_fee</li>";
				}
					...
					else
					{
						...
						if($wpbusdirman_pendingfeatured)
						{
							foreach($wpbusdirman_pendingfeatured as $wpbusdirman_pendingfeatureditem)
							{
								...
								echo "<td><a href=\"?page=wpbdman_c4&action=upgradefeatured&id=$wpbusdirman_pendingfeatureditem\">";
								_e("Upgrade","WPBDM");
								echo "</a> | <a href=\"?page=wpbdman_c4&action=cancelfeatured&id=$wpbusdirman_pendingfeatureditem\">";
								_e("Downgrade","WPBDM");
								echo "</td></tr>";
							}
						}
...
function wpbusdirman_list_categories()
{
	...
	wp_list_categories('taxonomy='.$wpbdmposttypecategory.'&title_li=&orderby='.$wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_7'].'&order='.$wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_8'].'&show_count='.$wpbdm_show_count.'&hide_empty='.$wpbdm_hide_empty.'&hierarchical='.$wpbdm_show_parent_categories_only);
...
function wpbusdirman_catpage_query()
{
	...
	//echo $mywpbdmcat;
	//die;
	//$mywpbdmcat = get_terms($wpbdmposttypecategory.'&orderby=count&hide_empty=0');


	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
	$args=array('post_status' => 'publish','post_type' => $wpbdmposttype,'paged'=>$paged,$wpbdmposttypecategory=> $mywpbdmcat,order_by=>'meta_key=sticky&meta_value=approved');
...
function wpbusdirman_indexpage_query()
{
	...
	$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
	$args=array('post_status' => 'publish','post_type' => $wpbdmposttype,'paged'=>$paged,order_by=>'meta_key=sticky&meta_value=approved' );
	query_posts($args);
	$wpbusdirman_stickyids=array();
...
function wpbusdirman_view_edit_delete_listing_button()
{
	...
	$wpbusdirman_permalink=get_permalink($wpbusdirman_gpid);?>
		...
		<div style="float:left;margin-right:20px;"><a href="<?php the_permalink(); ?>"><form method="get" action="<?php the_permalink(); ?>"><input type="submit" class="editlistingbutton" value="<?php _e("View","WPBDM");?>"/></form></a></div>
		...
		<div style="float:left;margin-right:20px;"><form method="post" action="<?php echo $wpbusdirman_permalink;?>"><input type="hidden" name="action" value="editlisting"/><input type="hidden" name="wpbusdirmanlistingid" value="<?php echo get_the_id();?>"/><input type="submit" class="editlistingbutton" value="<?php _e("Edit","WPBDM");?>"/></form></div>
		<div style="float:left;"><form method="post" action="<?php echo $wpbusdirman_permalink;?>"><input type="hidden" name="action" value="deletelisting"/><input type="hidden" name="wpbusdirmanlistingid" value="<?php echo get_the_id();?>"/><input type="submit" class="editlistingbutton" value="<?php _e("Delete","WPBDM");?>"/></form></div>
