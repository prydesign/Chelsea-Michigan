...
if(isset($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_51']) && !empty($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_51'])){$wpbdmposttypetagslug=$wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_51'];}else {$wpbdmposttypetagslug=$wpbdmposttypetags;}
	...
	//Register directory category taxonomy
	 register_taxonomy( $wpbdmposttypecategory, $wpbdmposttype, array( 
		'hierarchical' => true, 
		'label' => 'Directory Categories', 
		'update_count_callback' => '_update_post_term_count',
		'query_var' => $wpbdmposttypecategory, 
		'rewrite' => array('slug'=>$wpbdmposttypecategoryslug, 'with_front'=>false)
		 )
	 );
			...
			if($wpbusdirman_field_vals)
			{
				foreach($wpbusdirman_field_vals as $wpbusdirman_field_val)
				{
					...
					if($wpbusdirman_field_type == 1)
					{
						...
						echo "</p>";
						if ($wpbusdirman_field_value == array()) {
							echo "<input type=\"text\" name=\"wpbusdirman_field_label$wpbusdirman_field_label_association\" class=\"intextbox\" value=\"\">";
						} else {
					...
					{
						if($wpbusdirman_field_value == array()) {
						echo "<p class=\"wpbdmp\"><label>$wpbusdirman_field_label_name</label></p><textarea name=\"wpbusdirman_field_label$wpbusdirman_field_label_association\" class=\"intextarea\"></textarea>";
					} else {
					...
					elseif($wpbusdirman_field_type == 6) {
						...
						if($wpbusdirman_field_association == 'category') {
							$wpbusdirman_field_value = array();
							$terms =  wp_get_object_terms( $wpbdmlistingid, $wpbdmposttypecategory) ;
							foreach($terms as $term) {
								$wpbusdirman_field_value[] = $term->term_id;
							}
							...
							// echo '<pre>';
							// print_r($wpbusdirman_field_value);
							// echo '</pre>';
							if($mywpbdmcatlist)
							{
								...
								foreach($mywpbdmcatlist as $wpbusdirman_formseloption)
								{
									...
									if(in_array($mywpbdmcattermid, $wpbusdirman_field_value)){echo $wpbusdirmancheckedword;}
...
function wpbusdirmanui_homescreen ()
{
	...
	else
	{
			...
			elseif($wpbusdirmanaction == 'payment_step_1')
			{
				...
				// Submitting some goodness, lets add the meta infos before we upload images (damn this code is a mess)
				...
				add_post_meta($wpbusdirmanlistingpostid, "buyer_name", "Unknown", true) or update_post_meta($wpbusdirmanlistingpostid, "buyer_name", $first_name);
				add_post_meta($wpbusdirmanlistingpostid, "paymentgateway", "Check", true) or update_post_meta($wpbusdirmanlistingpostid, "paymentgateway", "Check");
			...
			if($wpbusdirmannumimgsleft <= 0)
			{
				...
				echo "</p>";
						if(get_post_meta($wpbusdirmanlistingpostid, "image", $single = true))
						{
							...
							{
								echo "<p>";
								_e("Click \"Submit\" to complete your directory submission.","WPBDM");
								echo "</p>";
						...
						else
						{
							if(!wpbusdirmancreatethumb($wpbusdirmanuploadedfilename,$wpbusdirmanuploaddir,$wpbusdirmanthumbnailwidth))
							{
								...
								add_post_meta($wpbusdirmanlistingpostid, "totalallowedimages", $wpbusdirmannumimgsallowed, true) or update_post_meta($wpbusdirmanlistingpostid, "totalallowedimages", $wpbusdirmannumimgsallowed);
									...
									add_post_meta($wpbusdirmanlistingID, "buyer_name", "Unknown", true) or update_post_meta($wpbusdirmanlistingID, "buyer_name", $first_name);
		...
		else
		{
			...
			else
			{
				...
				<?php else:
				 ...
				 _e("You do not currently have any listings in the directory, or your listing(s) are pending review","WPBDM");
...
function wpbusdirman_manage_paid()
{
	...
	if( isset($_REQUEST['action']) && !empty($_REQUEST['action']) && ($_REQUEST['action'] == 'setasnotpaid'))
	{
		if(isset($_REQUEST['id']) && !empty($_REQUEST['id']))
		{
			...
			delete_post_meta($wpbdmposttosetasnotpaid, "paymentstatus","paid");
			...
			add_post_meta($wpbdmposttosetasnotpaid, "paymentstatus","pending");
			...
			else
			{
				...
				echo "<th scope=\"col\" class=\"manage-column\">";
						...
						if($wpbusdirman_paidlistings)
						{
							...
							{
								$buyer_name=get_post_meta($wpbusdirman_paidlistingsitem, "buyer_name", true);
								...
								// Buyer
								echo "<td><span class=\"name\"><span>$buyer_name</span>";
								echo '</span>';
								echo ' &nbsp;<a class="meta_edit" href="javascript:void()">edit</a>';
								echo '<form class="meta_edit_form" action="javascript:void()" style="display: none;">';
								echo '<input type="hidden" name="id" value="'.$wpbusdirman_paidlistingsitem.'">';
								echo '<input type="hidden" name="meta" value="buyer_name">';
								echo '<input type="text" name="new_meta" value="'.$buyer_name.'">';
								echo '<input type="submit" value="Update">';
								echo '</form>';
								echo '<a class="cancel" style="display: none;" href="javascript:void()">cancel</a>';
								echo "</td>";
								
								// Email
								echo "<td><span class=\"name\"><span>$pemail</span>";
								echo '</span>';
								echo ' &nbsp;<a class="meta_edit" href="javascript:void()">edit</a>';
								echo '<form class="meta_edit_form" action="javascript:void()" style="display: none;">';
								echo '<input type="hidden" name="id" value="'.$wpbusdirman_paidlistingsitem.'">';
								echo '<input type="hidden" name="meta" value="payeremail">';
								echo '<input type="text" name="new_meta" value="'.$pemail.'">';
								echo '<input type="submit" value="Update">';
								echo '</form>';
								echo '<a class="cancel" style="display: none;" href="javascript:void()">cancel</a>';
								echo "</td>";
...
{
	global $wpbdmposttype,$wpbdmposttypecategory, $premier_listings, $enhanced_listings, $standard_listings, $post;
	$mywpbdmcat=get_query_var($wpbdmposttypecategory);
	//echo $mywpbdmcat;
	//die;
	...
	$args_premier = array(
	    ...
	    'meta_value'      => 1,
		 'paged'				 =>$paged,
		 $wpbdmposttypecategory => $mywpbdmcat,
	...
	$premier_listings = get_posts($args_premier);
	$enhanced_listings = get_posts($args_enhanced);
	$standard_listings = get_posts($args_standard);
}
	...
	if($wpbusdirman_config_options[$wpbusdirmanconfigoptionsprefix.'_settings_config_11'] == "yes")
	{
		$usingdefault=0;
			...
			{?>
				...
				<img src="<?php if($usingdefault != 1){?><?php echo $wpbdmimagesurl; ?>/<?php }?><?php echo $wpbusdirmanpostimagefeature;?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>" border="0" />
	...
	global $post,$wpbdmimagesurl;
		...
		{ ?>
			...
			<?php foreach($wpbusdirmanpostimages as $wpbusdirmanpostimage)
			{
				...
				{?>
					<img class="wpbdmthumbs" src="<?php echo $wpbdmimagesurl; ?>/<?php echo $wpbusdirmanpostimage;?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>" border="0">
				<?php
				}
			}?>
			</div>
